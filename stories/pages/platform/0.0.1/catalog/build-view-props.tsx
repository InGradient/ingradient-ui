import type { CatalogMobileViewMode, CatalogViewProps } from '@ingradient/platform-pages'
...
export function buildCatalogViewProps(
  ...
  const images = {
    ...
    onOpenDetail: (id: string) => s.setDetailImageId(id),
  ...
  if (isMobile) {
    const viewMode: CatalogMobileViewMode = s.viewMode === 'table' ? 'table' : 'grid'
    return {
      ...shared,
      isMobile: true,
      toolbar: { ...toolbar, viewMode, onChangeViewMode: s.setViewMode },
      mobile: {
  ...
  return {
    ...shared,
    isMobile: false,
    toolbar: { ...toolbar, viewMode: s.viewMode, onChangeViewMode: s.setViewMode },
    rightSidebar: {
