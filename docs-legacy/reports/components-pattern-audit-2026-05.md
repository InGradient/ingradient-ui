...
> **Status — archived historical audit ledger.** 이 문서는 audit 당시의 결정과 파일 위치를 보존한다. 이후 domain extraction 결과는 [Components Vs Patterns](../../docs/reference/components-vs-patterns.md), [`@ingradient/platform-pages`](../../packages/platform-pages/README.md), [Platform Story Contract](../../stories/pages/platform/0.0.1/README.md)를 현재 authority로 사용한다.

[Components Vs Patterns](../../docs/reference/components-vs-patterns.md) 기준으로 `src/components/` 전체를 1회 분류한 결과를 고정한다.
...
## Domain extraction epilogue

이 audit 이후 `DatasetTaskTag`, `SyncStatusChip`, `DatasetListItem`, Catalog gallery controls 등 제품 의미를 가진 composition은 `packages/platform-pages`로 이동했다. 따라서 위 round별 위치와 Storybook title은 역사적 실행 기록이며 현재 import 경로를 뜻하지 않는다. 현재 public boundary와 executable page documentation은 package README와 Platform `0.0.1` README/MIGRATION을 따른다.
